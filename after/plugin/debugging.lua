...
dap.configurations.cs = {
	{
		...
		program = function()
			...
			local logger = log.get_logger('kraytonian_vim')
			...
			if (Kraytonian_config.debug_dllPath ~= nill) then
				executable_path = Kraytonian_config.debug_dllPath
			end

			local result = vim.fn.input('Path to dll: ', executable_path, 'file')
			Kraytonian_config.debug_dllPath = result
